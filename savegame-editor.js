...
/* LIBRARIES */
function i18nGet(key, fallback, vars){
	...
	return{
		...
		addGlobalZone:function(f,t){
			...
			if(t)
				...
				span.innerHTML=i18nGet('drag.dropFiles','Drop files here');
...
function _tempFileLoadFunction(){
	if(SavegameEditor.checkValidSavegame()){
		...
		MarcDialogs.alert(i18nGet('errors.invalidSavegame','Invalid savegame file'));
	}
}
...
function closeFileConfirm(){
	MarcDialogs.confirm(i18nGet('confirm.discardChanges','All changes will be lost.'), function(){
		closeFile();
		MarcDialogs.close()
	});
}
